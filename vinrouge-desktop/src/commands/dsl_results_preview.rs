...
"##);
    ...
    if passed > 0 { summary.push(format!("<span style=\"font-size:10px;padding:2px 8px;border-radius:99px;background:rgba(74,222,128,0.12);color:#4ade80;\">{} passed</span>", passed)); }
    if failed > 0 { summary.push(format!("<span style=\"font-size:10px;padding:2px 8px;border-radius:99px;background:rgba(248,113,113,0.12);color:#f87171;\">{} failed</span>", failed)); }
...
fn count_results(results: &[serde_json::Value]) -> (usize, usize, usize, usize, usize, usize, usize) {
    let mut passed = 0;
    let mut failed = 0;
    let mut errors = 0;
    let mut samples = 0;
    ...
    for r in results {
        match r["kind"].as_str() {
            Some("assert") => {
                if r["passed"].as_bool() == Some(true) { passed += 1; } else { failed += 1; }
            }
            Some("error") => errors += 1,
            ...
            Some("section") => {
                ...
                samples += sa; charts += c; screens += sc; sections += se;
            }
            _ => {}
        }
    }
...
fn build_chart_option(chart_type: &str, labels: &[String], values: &[String]) -> String {
    let cats: Vec<String> = labels.iter().map(|l| format!("\"{}\"", l.replace('"', "\\\""))).collect();
    let vals: Vec<String> = values.iter().map(|v| v.to_string()).collect();

    match chart_type {
        "pie" => {
            let items: Vec<String> = labels.iter().zip(values.iter())
                .map(|(l, v)| format!("{{\"name\":\"{}\",\"value\":{}}}", l.replace('"', "\\\""), v))
                .collect();
            format!(
...
fn results_to_html(results: &[serde_json::Value], chart_idx: &mut usize) -> String {
    results.iter().map(|r| result_to_html(r, chart_idx)).collect()
}
